 ...
 */
class phpMorphy_Fsa_Sparse_Shm extends phpMorphy_Fsa {
    function walk($trans, $word, $readAnnot = true) {
        $__shm = $this->resource['shm_id']; $__offset = $this->resource['offset']; $fsa_start = $this->fsa_start;
        ...
        for($i = 0, $c = $GLOBALS['__phpmorphy_strlen']($word); $i < $c; $i++) {
			...
			list(, $trans) = unpack('V', shmop_read($__shm, $__offset + ($fsa_start + (((($trans >> 10) & 0x3FFFFF) + $char + 1) << 2)), 4));
        ...
        if($i >= $c) {
            ...
            if($readAnnot) {
                ...
                list(, $trans) = unpack('V', shmop_read($__shm, $__offset + ($fsa_start + ((($trans >> 10) & 0x3FFFFF) << 2)), 4));
    ...
    function readState($index) {
        ...
        list(, $trans) = unpack('V', shmop_read($__shm, $__offset + ($start_offset), 4));
        ...
        foreach($this->getAlphabetNum() as $char) {
            ...
            list(, $trans) = unpack('V', shmop_read($__shm, $__offset + ($start_offset + (($char) << 2)), 4));
...
// TODO: check term and empty flags at once i.e. $trans & 0x0300
    ...
    protected function readRootTrans() {
        ...
        list(, $trans) = unpack('V', shmop_read($__shm, $__offset + ($fsa_start + 4), 4));
    ...
    protected function readAlphabet() {
        ...
        return shmop_read($__shm, $__offset + ($this->header['alphabet_offset']), $this->header['alphabet_size']);
    }
    ...
    function getAnnot($trans) {
        ...
        $len = ord(shmop_read($__shm, $__offset + ($offset), 1));
        
        if($len) {
            $annot = shmop_read($__shm, $__offset + ($offset + 1), $len);
        } else {
            $annot = null;
        }
